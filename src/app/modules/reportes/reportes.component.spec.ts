...
describe('ReportesComponent', () => {
  ...
  beforeEach(async () => {
    ...
    mockVendedoresService.getDatosClientes.and.returnValue(
      of({ clientesAtendidos: [], nuevosClientes: [] })
    );
    mockVendedoresService.getDatosDistribucion.and.returnValue(
      of({ labels: [], data: [] })
    );
  ...
  it('should select vendedor and load indicators', () => {
    ...
    mockVendedoresService.getIndicadoresVendedor.and.returnValue(
      of(mockIndicadores)
    );
    ...
    expect(mockVendedoresService.getIndicadoresVendedor).toHaveBeenCalledWith(
      1
    );
  });
  ...
  it('should call chart creation methods when indicators are loaded', () => {
    ...
    mockVendedoresService.getDatosVentasHistoricas.and.returnValue(
      of([1, 2, 3, 4, 5, 6, 7])
    );
    mockVendedoresService.getDatosClientes.and.returnValue(
      of({
  ...
  it('should handle seleccionarVendedor with valid vendedor ID', () => {
    ...
    expect(component.vendedorSeleccionado).toEqual({
      id: 1,
      nombre: 'Test Vendedor',
      activo: true,
    });
  ...
  it('should handle different vendedor selection scenarios', () => {
    const testCases = [
      {
        value: 1,
        expected: { id: 1, nombre: 'Pedro Pablo Mercedes', activo: true },
      },
      {
        value: 2,
        expected: { id: 2, nombre: 'Juan Jose Torres', activo: true },
      },
      { value: 3, expected: { id: 3, nombre: 'María González', activo: true } },
      {
        value: 4,
        expected: { id: 4, nombre: 'Carlos Rodríguez', activo: true },
      },
    ];
  ...
  it('should handle different chart creation scenarios', () => {
    ...
    component.lineChartRef = {
      nativeElement: { getContext: () => ({}) },
    } as ElementRef;
    component.barChartRef = {
      nativeElement: { getContext: () => ({}) },
    } as ElementRef;
    ...
    expect(mockVendedoresService.getDatosVentasHistoricas).toHaveBeenCalledWith(
      1
    );
  ...
  it('should handle chart creation with missing chart refs', () => {
    ...
    expect(
      mockVendedoresService.getDatosVentasHistoricas
