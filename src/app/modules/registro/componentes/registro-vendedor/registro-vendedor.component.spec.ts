...
describe('RegistroVendedorComponent', () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [RegistroVendedorComponent, ReactiveFormsModule, NoopAnimationsModule],
      providers: [
        FormBuilder,
  ...
  it('should initialize form with default values', () => {
    ...
    expect(component.vendedorForm.get('rutaVisitas')?.value).toBe('Cliente A, Cliente B, Cliente C');
    expect(component.vendedorForm.get('correo')?.value).toBe('juan.perez@email.com');
  ...
  it('should show correct error messages for required fields', () => {
    ...
    expect(component.getFieldError('zona')).toBe('Debe seleccionar una zona asignada.');
    
    // Test ruta error
    rutaControl?.setValue('');
    rutaControl?.markAsTouched();
    expect(component.getFieldError('rutaVisitas')).toBe('Debe ingresar la ruta de visitas.');
  });
  ...
  it('should show correct error messages for invalid formats', () => {
    ...
    nombreControl?.setValue('Juan123');
    nombreControl?.markAsTouched();
    expect(component.getFieldError('nombre')).toBe('Solo se permiten caracteres alfabéticos y espacios');
    
    // Test email error
    correoControl?.setValue('invalid-email');
    correoControl?.markAsTouched();
    expect(component.getFieldError('correo')).toBe('Formato de correo inválido');
    
    // Test password error
    contrasenaControl?.setValue('password');
    contrasenaControl?.markAsTouched();
    expect(component.getFieldError('contrasena')).toBe('La contraseña debe tener al menos 8 caracteres e incluir letras y números');
  });

  it('should handle cancel action', () => {
    spyOn(component as any, 'createForm').and.returnValue(component.vendedorForm);
  ...
  it('should have correct getters for form controls', () => {
    ...
    expect(component.rutaVisitas).toBe(component.vendedorForm.get('rutaVisitas'));
  ...
  it('should mark all form controls as touched when form is invalid', () => {
    const markAsTouchedSpy = spyOn(component.vendedorForm.get('nombre')!, 'markAsTouched');
